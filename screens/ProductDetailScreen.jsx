...
const L = ({ c }) => <Text style={styles.sectionLabel}>{c}</Text>;

// ─── Calendario mejorado ──────────────────────────────────────────────────────
function CalPicker({ visible, onClose, value, onChange, accent = DS.primary }) {
  const [nav, setNav]         = useState(value ? new Date(value) : new Date());
  const [yearMode, setYearMode] = useState(false);
  React.useEffect(() => { if (visible) { setNav(value ? new Date(value) : new Date()); setYearMode(false); } }, [visible]);
        ...
        <View style={cS.card}>
          ...
          <View style={cS.grid}>
            {cells.map((d, i) => {
              ...
              return (
                <TouchableOpacity key={d} style={[cS.cell, s && [cS.cellS, {backgroundColor: accent}], !s&&t&&[cS.cellT,{borderColor:accent}]]}
...
const cS = StyleSheet.create({
  ...
  hdr:{padding:20,paddingBottom:14}, hdrY:{fontSize:12,color:'rgba(255,255,255,0.75)',fontWeight:'700'},
  hdrM:{fontSize:24,color:'#FFF',fontWeight:'900',lineHeight:28}, hdrD:{fontSize:12,color:'rgba(255,255,255,0.85)',marginTop:4},
  ...
  cellS:{borderRadius:20}, cellT:{borderRadius:20,borderWidth:1.5},
  dTxt:{fontSize:13,color:DS.text}, dTxtS:{color:'#FFF',fontWeight:'900'},
...
function CatModal({ visible, onClose, onSelect, currentCat, currentSub }) {
  ...
  React.useEffect(() => {
    ...
    else { const leg=DatabaseService.getDictionary(); const b={}; Object.keys(leg).forEach(k=>{b[k]={tags:leg[k],subcategories:{}};}); setDict(b); }
    setSelCat(currentCat||null); setStep('cat');
  }, [visible, currentCat]);
  const cats=Object.keys(dict), catD=selCat?dict[selCat]:null, subs=catD?Object.keys(catD.subcategories||{}):[];
  const pickCat=(cat) => { if (Object.keys(dict[cat]?.subcategories||{}).length>0){setSelCat(cat);setStep('sub');}else{onSelect(cat,null);onClose();} };
  return (
      ...
      <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={onClose}>
        <View style={styles.modalSheet}>
          ...
          <View style={styles.modalHeader}>
            {step==='sub' && <TouchableOpacity onPress={()=>setStep('cat')} style={styles.modalBackBtn}><Icon name="arrow-left" size={16} color={DS.text}/></TouchableOpacity>}
            <Text style={styles.modalTitle}>{step==='cat'?'Categoría':`${selCat} — Subcategoría`}</Text>
            <TouchableOpacity onPress={onClose} style={styles.modalCloseBtn}><Icon name="x" size={16} color={DS.textMed}/></TouchableOpacity>
          </View>
          <FlatList data={step==='cat'?cats:['Sin subcategoría',...subs]} keyExtractor={i=>i}
            renderItem={({item}) => {
              const isN=item==='Sin subcategoría';
              const isCurr=step==='cat'?item===currentCat:(!isN&&item===currentSub)||(isN&&!currentSub);
              const hasSubs=step==='cat'&&Object.keys(dict[item]?.subcategories||{}).length>0;
              return (
                <TouchableOpacity style={[styles.modalItem, isCurr&&styles.modalItemActive]}
                  onPress={()=>step==='cat'?pickCat(item):(onSelect(selCat,isN?null:item),onClose())}>
                  <View style={{flex:1}}>
                    <Text style={[styles.modalItemTxt, isCurr&&{color:DS.blue,fontWeight:'800'}]}>{item}</Text>
                    {step==='cat'&&dict[item]?.tags?.length>0&&<Text style={styles.modalItemSub} numberOfLines={1}>{dict[item].tags.slice(0,5).join(' · ')}</Text>}
                    {hasSubs&&<Text style={[styles.modalItemSub,{color:DS.blue}]}>{Object.keys(dict[item].subcategories).length} subcategorías</Text>}
                  </View>
...
function SoldModal({ visible, onClose, product, onConfirm }) {
  ...
  React.useEffect(() => { if (visible) { setPrice(String(product?.price||'')); setDate(new Date().toISOString()); } }, [visible]);
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
        ...
        <TouchableOpacity activeOpacity={1} style={styles.soldSheet}>
          ...
          <View style={styles.soldBody}>
            <View style={styles.soldPriceRow}>
              <View style={styles.soldPriceCard}><Text style={styles.soldPriceLbl}>PRECIO PUBLICADO</Text><Text style={styles.soldPriceVal}>{listed}€</Text></View>
              {profit!==null&&<View style={[styles.soldPriceCard,{backgroundColor:Number(profit)>=0?DS.successBg:'#FFEBEC'}]}>
                <Text style={styles.soldPriceLbl}>BENEFICIO</Text>
                <Text style={[styles.soldPriceVal,{color:Number(profit)>=0?DS.success:DS.danger}]}>{Number(profit)>=0?'+':''}{profit}€</Text>
              </View>}
            </View>
            <Text style={styles.soldFieldLbl}>PRECIO REAL DE VENTA (€) *</Text>
            <View style={[styles.soldInputRow, price&&{borderColor:DS.success}]}>
              <TextInput value={price} onChangeText={setPrice} placeholder={String(listed)}
                placeholderTextColor={DS.textLow} keyboardType="decimal-pad"
                style={styles.soldInput} autoFocus/>
            ...
            <TouchableOpacity style={[styles.soldConfirm,!price&&{backgroundColor:DS.textLow}]} onPress={confirm} disabled={!price}>
      ...
      <CalPicker visible={showCal} onClose={()=>setCal(false)} value={date} onChange={setDate} accent={DS.success}/>
    </Modal>
  );
}
...
export default function ProductDetailScreen({ route, navigation }) {
  ...
  const catTags = useMemo(() => {
    const full=DatabaseService.getFullDictionary()||{}, catD=full[editForm.category||product.category];
    if (!catD) return [];
    const sk=editForm.subcategory||product.subcategory, st=sk?(catD.subcategories?.[sk]?.tags||[]):[];
  ...
  const handleSave = useCallback(() => {
    ...
    if (DatabaseService.updateProduct(up)){setProduct(up);setEditing(false);span.end({});}
    else {span.fail(new Error('false'));Alert.alert('Error','No se pudo guardar.');}
  }, [editForm, product]);
  ...
  const handleSold = (soldPrice, soldDateReal) => {
    if (DatabaseService.markAsSold(product.id,soldPrice,soldDateReal,false)){
      ...
      setProduct(up); setSoldModal(false);
      ...
      Alert.alert('¡Vendido! 🎉',`"${product.title}" marcado por ${soldPrice}€.`,[
        {text:'Ver historial',onPress:()=>navigation.navigate('History')},{text:'OK'}]);
    }
  };

  const sC=statusInfo.isCrit?DS.danger:statusInfo.isCold?DS.warning:DS.success;
    ...
    <>
      <View style={styles.topRow}>
        ...
        <View style={styles.catPill}>
          ...
          <Text style={styles.catPillTxt}>{product.category||'Sin cat.'}</Text>
          {product.subcategory&&<><Text style={styles.catSep}>›</Text><Text style={[styles.catPillTxt,{color:DS.textMed}]}>{product.subcategory}</Text></>}
        </View>
      </View>
      <View style={styles.titleRow}>
        <Text style={styles.titleTxt} numberOfLines={3}>{product.title}</Text>
        <View style={styles.pricePill}><Text style={styles.pricePillTxt}>{product.price}€</Text></View>
      </View>
      {product.isBundle&&<View style={styles.bundleBadge}><Icon name="package" size={11} color={DS.purple}/><Text style={styles.bundleTxt}>VENTA EN LOTE</Text></View>}
      <View style={[styles.statusBar,{borderColor:sC+'40',backgroundColor:sC+'0E'}]}>
        ...
        {product.repostCount>0&&<View style={styles.repostChip}><Icon name="refresh-cw" size={9} color={DS.blue}/><Text style={styles.repostTxt}>{product.repostCount}× resubido</Text></View>}
      </View>
      <View style={styles.statsRow}>
        {[{icon:'eye',lbl:'VISTAS',val:product.views||0,c:DS.blue},{icon:'heart',lbl:'FAVS',val:product.favorites||0,c:DS.danger},{icon:'clock',lbl:'DÍAS',val:statusInfo.daysOld,c:sC}].map(s=>(
          <View key={s.lbl} style={styles.statCard}><Icon name={s.icon} size={16} color={s.c}/><Text style={[styles.statVal,{color:s.c}]}>{s.val}</Text><Text style={styles.statLbl}>{s.lbl}</Text></View>
        ))}
      </View>
      {product.priceHistory?.length>0&&(
        <View style={styles.histBox}>
          <View style={styles.histHdr}><Icon name="trending-down" size={11} color={DS.primary}/><Text style={styles.histLbl}>HISTORIAL DE PRECIO</Text></View>
          {product.priceHistory.slice(-3).map((h,i)=>(
            <View key={i} style={styles.histRow}><Text style={styles.histTxt}>{h.oldPrice}€ → {h.newPrice||h.price}€</Text><Text style={styles.histDate}>{fmtS(h.date)}</Text></View>
          ))}
        </View>
      )}
      <View style={styles.dateBar}>
        <View style={[styles.dateIcon,{backgroundColor:DS.primaryBg}]}><Icon name="upload" size={13} color={DS.primary}/></View>
        <View><Text style={styles.dateLbl}>Fecha de subida original</Text><Text style={styles.dateVal}>{fmt(product.firstUploadDate||product.createdAt)}</Text></View>
      </View>
      {catTags.length>0&&(
        <View style={styles.tagsSection}>
          <Text style={styles.sectionLabel}>TAGS DE CATEGORÍA</Text>
          <View style={styles.tagsCloud}>{catTags.map(t=><View key={t} style={styles.tag}><Text style={styles.tagTxt}>{t}</Text></View>)}</View>
        </View>
      )}
      ...
      <View style={styles.actionsRow}>
        <TouchableOpacity style={styles.btnEdit} onPress={()=>setEditing(true)}>
          <Icon name="edit-3" size={16} color={DS.text}/><Text style={styles.btnEditTxt}>Editar</Text>
        </TouchableOpacity>
        {product.status!=='sold'?(
          <TouchableOpacity style={styles.btnVendido} onPress={()=>setSoldModal(true)}>
            <Icon name="check-circle" size={16} color="#FFF"/><Text style={styles.btnVendidoTxt}>VENDIDO</Text>
          </TouchableOpacity>
        ):(
          <View style={[styles.btnEdit,{backgroundColor:DS.successBg,flex:2,borderColor:DS.success+'40'}]}>
            <Icon name="check-circle" size={16} color={DS.success}/><Text style={[styles.btnEditTxt,{color:DS.success}]}>Ya vendido</Text>
          </View>
        )}
      </View>
    </>
  ...
  const renderEdit = () => (
    <>
      <View style={styles.editBanner}>
        ...
        <View style={{flex:1}}><Text style={styles.editTitle}>Editar datos permanentes</Text><Text style={styles.editSub}>Se conservan al importar JSON de Vinted</Text></View>
      </View>
      ...
      <TouchableOpacity style={styles.catSelector} onPress={()=>setCatModal(true)}>
        <View style={[styles.catSelIcon,{backgroundColor:DS.blueBg}]}><Icon name="tag" size={14} color={DS.blue}/></View>
        <View style={{flex:1}}>
      ...
      {(()=>{
        const full=DatabaseService.getFullDictionary()||{}, catD=full[editForm.category];
        ...
        return <View style={styles.tagPreview}><Text style={styles.tagPreviewLbl}>Tags sugeridos:</Text><View style={styles.tagsCloud}>{tags.map(t=><View key={t} style={styles.tag}><Text style={styles.tagTxt}>{t}</Text></View>)}</View></View>;
      })()}
      <Text style={styles.sectionLabel}>FECHA DE SUBIDA ORIGINAL</Text>
      <TouchableOpacity style={styles.datePickBtn} onPress={()=>setShowCal(true)}>
        <View style={[styles.datePickIcon,{backgroundColor:DS.primaryBg}]}><Icon name="calendar" size={16} color={DS.primary}/></View>
        <View style={{flex:1}}><Text style={styles.datePickVal}>{fmt(editForm.firstUploadDate)}</Text><Text style={styles.datePickHint}>Toca para cambiar</Text></View>
        <Icon name="chevron-right" size={16} color={DS.textLow}/>
      </TouchableOpacity>
      <View style={styles.editActions}>
        <TouchableOpacity style={styles.btnSave} onPress={handleSave}>
          <Icon name="save" size={15} color="#FFF"/><Text style={styles.btnSaveTxt}>Guardar cambios</Text>
        </TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      <SoldModal visible={showSoldModal} onClose={()=>setSoldModal(false)} product={product} onConfirm={handleSold}/>
      <CatModal visible={showCatModal} onClose={()=>setCatModal(false)} currentCat={editForm.category} currentSub={editForm.subcategory} onSelect={(cat,sub)=>setEditForm(f=>({...f,category:cat,subcategory:sub}))}/>
      <CalPicker visible={showCal} onClose={()=>setShowCal(false)} value={editForm.firstUploadDate} onChange={iso=>setEditForm(f=>({...f,firstUploadDate:iso}))} accent={DS.primary}/>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          {product.images?.[0]?<Image source={{uri:product.images[0]}} style={styles.heroImg} resizeMode="cover"/>:<View style={[styles.heroImg,styles.heroPlaceholder]}><Icon name="image" size={52} color={DS.textLow}/></View>}
          <TouchableOpacity style={styles.backBtn} onPress={()=>navigation.goBack()}><Icon name="arrow-left" size={20} color={DS.text}/></TouchableOpacity>
          <View style={[styles.heroBadge,{backgroundColor:sC}]}><Text style={styles.heroBadgeTxt}>{sL}</Text></View>
        </View>
...
const styles = StyleSheet.create({
  ...
  heroBadge:{position:'absolute',bottom:20,right:20,paddingHorizontal:14,paddingVertical:7,borderRadius:20},
  ...
  catPillTxt:{fontSize:10,fontWeight:'800',color:DS.blue}, catSep:{fontSize:10,color:DS.textLow},
  ...
  bundleBadge:{flexDirection:'row',alignItems:'center',gap:6,backgroundColor:'#F0EFFE',paddingHorizontal:10,paddingVertical:5,borderRadius:10,alignSelf:'flex-start',marginBottom:10},
  bundleTxt:{fontSize:10,fontWeight:'900',color:DS.purple},
  statusBar:{flexDirection:'row',alignItems:'center',gap:8,borderWidth:1,borderRadius:14,padding:12,marginBottom:14},
  statusDot:{width:8,height:8,borderRadius:4}, statusLbl:{fontSize:12,fontWeight:'900'},
  statusDays:{fontSize:11,color:DS.textMed,marginLeft:'auto'},
  repostChip:{flexDirection:'row',alignItems:'center',gap:4,backgroundColor:DS.blueBg,paddingHorizontal:7,paddingVertical:3,borderRadius:8},
  repostTxt:{fontSize:9,fontWeight:'800',color:DS.blue},
  statsRow:{flexDirection:'row',gap:10,marginBottom:14},
  statCard:{flex:1,backgroundColor:DS.bg,borderRadius:16,padding:12,alignItems:'center',gap:4},
  statVal:{fontSize:20,fontWeight:'900',fontFamily:DS.mono},
  statLbl:{fontSize:8,fontWeight:'900',color:DS.textLow,letterSpacing:1},
  histBox:{backgroundColor:DS.primaryBg,borderRadius:14,padding:14,marginBottom:14,borderWidth:1,borderColor:DS.primary+'25'},
  ...
  histTxt:{fontSize:12,fontWeight:'700',color:DS.text}, histDate:{fontSize:10,color:DS.textMed},
  dateBar:{flexDirection:'row',alignItems:'center',gap:12,backgroundColor:DS.bg,borderRadius:14,padding:12,marginBottom:14},
  dateIcon:{width:36,height:36,borderRadius:10,justifyContent:'center',alignItems:'center'},
  dateLbl:{fontSize:10,color:DS.textMed,marginBottom:2}, dateVal:{fontSize:13,fontWeight:'800',color:DS.text},
  ...
  sectionLabel:{fontSize:10,fontWeight:'900',color:DS.textLow,letterSpacing:1.5,textTransform:'uppercase',marginBottom:6,marginTop:14},
  descTxt:{fontSize:14,color:DS.textMed,lineHeight:22,marginBottom:16},
  actionsRow:{flexDirection:'row',gap:10,marginTop:16},
  btnEdit:{flex:1,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,backgroundColor:DS.bg,borderWidth:1,borderColor:DS.border,paddingVertical:15,borderRadius:18},
  btnEditTxt:{fontSize:13,fontWeight:'800',color:DS.text},
  btnVendido:{flex:2,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:8,backgroundColor:DS.success,paddingVertical:15,borderRadius:18},
  btnVendidoTxt:{fontSize:13,fontWeight:'900',color:'#FFF',letterSpacing:0.5},
  editBanner:{flexDirection:'row',alignItems:'center',gap:12,backgroundColor:DS.primaryBg,borderRadius:16,padding:14,marginBottom:16,borderWidth:1,borderColor:DS.primary+'20'},
  editTitle:{fontSize:15,fontWeight:'900',color:DS.text}, editSub:{fontSize:11,color:DS.textMed,marginTop:2},
  ...
  catSelVal:{fontSize:15,fontWeight:'800',color:DS.text}, catSelSub:{fontSize:11,color:DS.textMed,marginTop:2},
  ...
  datePickVal:{fontSize:15,fontWeight:'700',color:DS.text}, datePickHint:{fontSize:10,color:DS.textLow,marginTop:2},
  editActions:{flexDirection:'row',gap:10,marginTop:24},
  btnSave:{flex:2,flexDirection:'row',justifyContent:'center',alignItems:'center',gap:8,backgroundColor:DS.text,padding:16,borderRadius:18},
  btnSaveTxt:{color:'#FFF',fontWeight:'900',fontSize:14},
  btnCancel:{flex:1,justifyContent:'center',alignItems:'center',backgroundColor:DS.bg,borderWidth:1,borderColor:DS.border,padding:16,borderRadius:18},
  btnCancelTxt:{color:DS.textMed,fontWeight:'800'},
  modalOverlay:{flex:1,backgroundColor:'#00000060',justifyContent:'flex-end'},
  modalSheet:{backgroundColor:DS.white,borderTopLeftRadius:28,borderTopRightRadius:28,paddingHorizontal:20,paddingTop:12,maxHeight:'82%',paddingBottom:30},
  ...
  modalBackBtn:{width:34,height:34,borderRadius:17,backgroundColor:DS.bg,justifyContent:'center',alignItems:'center'},
  modalCloseBtn:{width:30,height:30,borderRadius:15,backgroundColor:DS.bg,justifyContent:'center',alignItems:'center'},
  modalTitle:{flex:1,fontSize:17,fontWeight:'900',color:DS.text},
  modalItem:{flexDirection:'row',alignItems:'center',paddingVertical:13,borderBottomWidth:1,borderBottomColor:DS.bg,gap:10},
  modalItemActive:{backgroundColor:DS.blueBg,paddingHorizontal:10,marginHorizontal:-10,borderRadius:10},
  modalItemTxt:{fontSize:15,color:DS.text,fontWeight:'600'},
  modalItemSub:{fontSize:10,color:DS.textMed,marginTop:2},
  soldSheet:{backgroundColor:DS.white,borderTopLeftRadius:28,borderTopRightRadius:28,paddingBottom:30},
  soldHdr:{flexDirection:'row',alignItems:'center',gap:12,padding:20,backgroundColor:DS.successBg,borderTopLeftRadius:28,borderTopRightRadius:28,borderBottomWidth:1,borderBottomColor:DS.success+'25'},
  soldIconWrap:{width:44,height:44,borderRadius:22,backgroundColor:'#FFF',justifyContent:'center',alignItems:'center'},
  soldHdrTitle:{fontSize:17,fontWeight:'900',color:DS.text}, soldHdrSub:{fontSize:12,color:DS.textMed,marginTop:2},
  soldBody:{padding:20},
  soldPriceRow:{flexDirection:'row',gap:10,marginBottom:20},
  soldPriceCard:{flex:1,backgroundColor:DS.bg,borderRadius:14,padding:12},
  ...
  soldFieldLbl:{fontSize:9,fontWeight:'900',color:DS.textLow,letterSpacing:1.5,textTransform:'uppercase',marginBottom:8},
  soldInputRow:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:DS.bg,borderWidth:2,borderColor:DS.border,borderRadius:16,paddingHorizontal:16,paddingVertical:10,marginBottom:16},
  soldInput:{flex:1,fontSize:28,fontWeight:'900',color:DS.text,fontFamily:DS.mono,textAlign:'center'},
  soldInputEuro:{fontSize:22,fontWeight:'900',color:DS.textMed},
  soldDateRow:{flexDirection:'row',alignItems:'center',gap:10,backgroundColor:DS.bg,borderRadius:14,padding:14,marginBottom:20,borderWidth:1,borderColor:DS.border},
  soldDateTxt:{flex:1,fontSize:14,fontWeight:'700',color:DS.text},
  soldConfirm:{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10,backgroundColor:DS.success,borderRadius:18,padding:18},
